...
router.get(
  "/forgot-password",
  query("email", "Invalid email").notEmpty().trim().isEmail(),
  async (/**@type {e.Request} */ req, /**@type {e.Response} */ res) => {
    try {
      const result = validationResult(req);
      if (!result.isEmpty())
      ...
      if (!state)
        return res.status(400).send({ message: "Couldn't send email" });
...
router.get(
  "/reset",
  query("token", "Invalid token").notEmpty().trim(),
  async (req, res) => {
    try {
      const result = validationResult(req);
      if (!result.isEmpty())
        return res.status(400).send({ message: result.array()[0].msg });
      res.sendFile("reset.html", { root: config.App.public });
    } catch (err) {
      return res.status(500).send({ message: err.message });
    }
  }
...
router.post(
  ...
  async (/**@type {e.Request} */ req, /**@type {e.Response} */ res) => {
    try {
      ...
      if (!response)
        return res.status(400).send({ message: "Couldn't reset password" });
...
router.get(
  ...
  (/**@type {e.Request} */ req, /**@type {e.Response} */ res) => {
    try {
      ...
      authController
        .verifyEmail(decoded.id)
        .then((state) => res.send("<b>Success!</b>"))
        .catch((err) => {
