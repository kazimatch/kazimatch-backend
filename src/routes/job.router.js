...
router.get("/all/talent", authMiddleware, async (req, res) => {
  try {
    res.json(await jobController.getTalentPool(req.user.id));
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
});
...
router.patch("/:id", authMiddleware, async (req, res) => {
  try {
    res.json(
      await jobController.updateJob(req.user.id, req.params.id, req.body)
    );
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.get("/:id/applications", authMiddleware, async (req, res) => {
  try {
    res.json(
      await jobController.getJobApplications(
        req.user.id,
        req.params.id,
        req.query
      )
...
router.post("/:id/applications", authMiddleware, async (req, res) => {
  try {
    const application = await jobController.addJobApplication(
      req.user.id,
      req.params.id,
      req.body
    );
...
router.patch("/:id/applications/:appId", authMiddleware, async (req, res) => {
  try {
    const application = await jobController.updateApplication(
      req.params.id,
      req.params.appId,
      req.body
    );
...
router.delete("/:id/applications/:appId", authMiddleware, async (req, res) => {
  try {
    const application = await jobController.deleteApplication(
      req.params.id,
      req.user.id,
      req.params.appId
    );
